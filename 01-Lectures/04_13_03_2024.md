...
Where to store key? İşlemcinin içinde, in xmm registers (not memory)
...
Authorization: Yetkilendirme. Kullanıcıların neye erişebileceğini belirleme. Örneğin 3D Secure de gelen mesajı okuyabilmek için bir banka uygulamasına izin vermemiz buna örnektir.
...
* Bu kullanıcını UID (Unique Integer ID) si 0 dır.
