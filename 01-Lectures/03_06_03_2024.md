...
# CAPN CRUNCH Vakası
1971 yılında meydana gelmiş
* Şehir içerisinde görüşme bedava. Fakat eyaletler arası görüşme parayla.
* AT&T bir telekominikasyon firması olarak bu işi opere etmektedir.
* Telefonu açtığında karşı taraf eğer eyaletler arası ise bir switch ten geçiyor.
* Eyaletler arası olduğunu anlamak için switch 120 Hz civarında bir ses ile kontrol ediyor.
* Aynı hat üzerinden kontrol ve data (kullanıcı konuşması) gitmiş oluyor.
* CAPN CRUNCH diye bir düdük icat edilerek bedava eyaletler arası konuşma yapılabiliyor.

Buna benzer bir hata işlemcilerin ve sistemlerin tasarımı sırasında yapılmış olup, geriye alması çok zor bir duruma gelmiştir. Tüm major sistemler aynı altyapıyı as is olarak kullanmaktadır.
...
* Intel de Last Branch Recording (LBR). En son nereden gelindiğinin bilgisini tutar.
...
* Gaz kaçağı sırasında kanaryalar daha çabuk bayılır.

* Canary dediğimiz bölüm stack in bir bölümünü kontrol için ayırmak ve burada bir değişiklik olduğunu tespit etmek için kullanılıyor.
...
* Canary nin 32 bit olduğunu düşünürsek bunun ilk 8 bit ini değiştirdiğimde uygulama crash oldu mu olmadı mı bakarım.
    ...
    * Bunu engellemek için windows tarafında ilk 3 server restartından sonra servis çalışmayı durdurur.
    * Manuel müdahale ile çalıştırıldığında canary değişecektir.
